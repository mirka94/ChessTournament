...
public class Simulator {
	...
	static String Simulate(int players, int rounds) {
		...
		Comparator<Player> playersLComp = new Comparator<Player>() {
			@Override
			public int compare(Player o1, Player o2) {
				if(o1.getPoints()>o2.getPoints()) return -1;
				else if(o1.getPoints()<o2.getPoints()) return 1;
				if(o1.getSonnebornBerner()>o2.getSonnebornBerner()) return -1;
				if(o1.getSonnebornBerner()<o2.getSonnebornBerner()) return 1;
		...
		for(int i=0; i<players; ++i) {
			Competitor c = RandomPlayer();
			Player pl = Player.createPlayerWithFirstnameLastname(c.getName(), c.getSurname());
			pl.setRating(ratings[c.getChessCategory()]);
			t.addPlayer(pl);
			playersL.add(pl);
		...
		for(int i=0; i<rounds; ++i) {
			...
			for(Pairing pairing : round.pairings) {
				Player black = pairing.getBlack(), white = pairing.getWhite();
				toOutput+="Białymi gra "+white.getFirstname()+" "+white.getLastname()+", ";
				toOutput+="czarnymi "+black.getFirstname()+" "+black.getLastname()+", ";
				int score = rn.nextInt(9);
				if(score<4) {// 0 1 2 3
					toOutput+="wygrywaja biale \n";
					pairing.whiteWins();
				} else if(score>5) {// 5 6 7 8
					toOutput+="wygrywaja czarne \n";
					pairing.blackWins();
				} else { // 4 
					toOutput+="partia konczy sie remisem \n";
					pairing.draw();
				}
			}
			if(round.hasBye()) {
				Player pl = round.bye;
				toOutput+="Pauzuje "+pl.getFirstname()+" "+pl.getLastname()+"\n";
			}
		}
	...
	static Competitor RandomPlayer() {
		Random rn = new Random();
	...
	private final static String[] imiona = {
			"Arek",
			"Arkadiusz",
			"Artek",
			"Artur",
